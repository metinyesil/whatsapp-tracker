...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  ...
  if (message.online) {
    if (lastLog !== 'online') {
      ...
      timeElement.textContent = "Son çevrimiçi zamanı: " + message.time;
  ...
  for (let i = logMessages.length - 1; i >= 0; i--) {
    ...
    if (log.online) {
      logItem.textContent = `Çevrimiçi oldu: ${log.timestamp.toLocaleString()}`;
    } else {
      logItem.textContent = `Çevrimdışı oldu: ${log.timestamp.toLocaleString()}`;
    }
